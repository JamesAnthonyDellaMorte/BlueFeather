...
void setBitmapFormat(BitmapObject *sprite, Texture *timg, u16 *palette) {
    const u8* textureBytes = (const u8*)timg;
    const u16 flags = hm64ReadRawU16(textureBytes + 2);
    const u16 width = hm64ReadRawU16(textureBytes + 4);
    const u16 height = hm64ReadRawU16(textureBytes + 6);

    setBitmapFormatAndSize(sprite, palette);
    
    // Raw texture assets store an 8-byte header followed by texel data.
    sprite->timg = (u8*)(textureBytes + 8);
    
    // bytes 4-8 = width and height (16 bit) (byte swapped)
    sprite->width = swap16(width);  
    sprite->height = swap16(height);
...
void setBitmapFormatAndSize(BitmapObject* sprite, u16* palette) {
    ...
    u32 padding[5];
    const u16 flags = hm64ReadRawU16((u8*)palette + 2);
    
    // skip header
    sprite->pal = (u16*)((u8*)palette + 4);
...
void nuGfxInit(void) {
    ...
    // PC port: Allocate PC memory for framebuffers instead of using N64 addresses
    static u16* pcFrameBuf[3];
    static int frameBufInitialized = 0;
    if (!frameBufInitialized) {
        pcFrameBuf[0] = (u16*)calloc(320 * 240, sizeof(u16));
        pcFrameBuf[1] = (u16*)calloc(320 * 240, sizeof(u16));
        pcFrameBuf[2] = (u16*)calloc(320 * 240, sizeof(u16));
        frameBufInitialized = 1;
    }
    nuGfxSetCfb(pcFrameBuf, 3);

    nuGfxSetZBuffer((u16*)calloc(320 * 240, sizeof(u16)));
