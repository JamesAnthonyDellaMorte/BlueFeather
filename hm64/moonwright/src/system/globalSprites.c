...
bool dmaSprite(u16 index, 
    HM64RomAddr romTextureStart, HM64RomAddr romTextureEnd, 
    HM64RomAddr romAssetsIndexStart, HM64RomAddr romAssetsIndexEnd, 
    HM64RomAddr romSpritesheetIndexStart, HM64RomAddr romSpritesheetIndexEnd, 
...
bool setSpriteType1(u16 spriteIndex, u32* animationIndexPtr, u32* spritesheetIndexPtr, u32* paletteIndexPtr, u8* spriteToPaletteMappingPtr, HM64RomAddr romTexturePtr, u8* texturePtr, u8* texture2Ptr) {
...
bool startSpriteAnimation(u16 spriteIndex, u16 animationIndex, u8 animationModeOrFrameIndex) {
    ...
    if (spriteIndex < MAX_SPRITES) {
        ...
        if (globalSprites[spriteIndex].stateFlags & SPRITE_ACTIVE) {
            ...
            if (hm64ReadIndexedU32(globalSprites[spriteIndex].animationIndexPtr, animationIndex) !=
                hm64ReadIndexedU32(globalSprites[spriteIndex].animationIndexPtr, animationIndex + 1)) {
...
void setTotalAnimationFrames(AnimationFrameMetadata* animationFrameMetadata, const void* animationAsset) {
    const u8* bytes = (const u8*)animationAsset;

    // Skip the 4-byte animation header and read the host-order frame count.
    animationFrameMetadata->objectCount = hm64ReadRawLEU16(bytes + 4);
}
...
static void setAnimationFrameMetadata(AnimationFrameMetadata* animationFrameMetadata, const void* animationFrameMetadataPtr) {
    ...
    animationFrameMetadata->frameDuration = bytes[2];
    animationFrameMetadata->audioTrigger = bytes[3];
}
...
static void setBitmapMetadata(BitmapMetadata* ptr, const void* data) {
    ...
    ptr->unk_2 = 0;
    ptr->anchorX = hm64ReadRawLES16(bytes + 4);
    ptr->anchorY = hm64ReadRawLES16(bytes + 6);
}
...
u8* getAnimationFrameMetadataPtr(u16 index, void* table) {
    ...
    return (u8*)table + hm64ReadIndexedU32(arr, index);
...
static AnimationFrameMetadata* getAnimationFrameMetadataPtrFromFrame(u16 currentFrame, u16* animationFrameMetadataPtr) {
    ...
    while (i < currentFrame) {
        setAnimationFrameMetadata(&animationFrameMetadata, cursor);

        // Each frame record is a 4-byte header followed by N 8-byte bitmap metadata entries.
        cursor += 4 + (animationFrameMetadata.objectCount * 8);
        i++;
    }
    
    return (AnimationFrameMetadata*)cursor;
