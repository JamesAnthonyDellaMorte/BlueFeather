...
u8* setSpriteDMAInfo(u16 index, u32 *spritesheetIndex, u8 *timg, u8 *romAddr) {
    ...
    u32 offset = hm64ReadIndexedU32(spritesheetIndex, index);
...
u32 getTextureLength(u16 spriteIndex, u32 spritesheetIndex[]) {
    ...
    u32 spriteOffset = hm64ReadIndexedU32(spritesheetIndex, spriteIndex);
    
    // check if next sprite has same address (some sprites are reused with different palettes)
    if (hm64ReadIndexedU32(spritesheetIndex, counter) == spriteOffset) {
        
        while (hm64ReadIndexedU32(spritesheetIndex, counter) == spriteOffset) {
            counter++;
        } 
        
    }
    
    return hm64ReadIndexedU32(spritesheetIndex, counter) - spriteOffset;
...
void updateBitmaps(void) {
    ...
    if (updateCount < 10) {
        printf("[SPRITE] updateBitmaps: frame=%d, activeBitmaps=%d\n", updateCount, activeCount);
        updateCount++;
    }
