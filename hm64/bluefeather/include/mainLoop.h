...
// Use void* return type to match original game code signature
// Even though callbacks don't actually return meaningful values
typedef void* (*MainLoopCallback)();

extern void mainLoop();

extern u32 setMainLoopCallbackFunctionIndex(u16 callbackIndex); 
extern bool registerMainLoopCallback(u16 index, MainLoopCallback func);

extern u8 gfxRetraceCallback(int);
extern void gfxBufferSwap(void *gfxTask);
extern void gfxPreNMICallback(void);

// noOpCallback must return void* to match MainLoopCallback signature
extern void* noOpCallback(void);

extern volatile u16 mainLoopCallbackCurrentIndex;
extern MainLoopCallback mainLoopCallbacksTable[MAIN_LOOP_CALLBACK_FUNCTION_TABLE_SIZE];
